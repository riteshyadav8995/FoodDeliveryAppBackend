...
async function getCart(userId){
 ...
 async function modifyCart(userId,productId,shouldAdd= true){
    ...
    cart.items.forEach(item=>{
            ...
            if(shouldAdd){
                 if(product.quantity>=item.quantity +1)
                    item.quantity+=quantityValue;
                     ...
                     throw new AppError("the quantity of item requested is not available",404);
            }
            else{
                if(item.quantity>0){
                    item.quantity+=quantityValue;
                    if(item.quantity==0){
                      cart.items=cart.items.filter(item=>item.products._id !== productId);
                      foundProduct=true;
                      return;
                    }
                }
     ...
     await cart.save(); // changes persist karne lagenge
